...
try:
    ...
    # 记录导入失败原因，方便在 Streamlit Cloud 上直观看到真实报错（而不是误以为没安装）
    ZHIPUAI_IMPORT_ERROR = repr(_zhipu_import_err)
    ZhipuAI = None  # type: ignore
else:
    ZHIPUAI_IMPORT_ERROR = ""
...
def _format_deepseek_request_error(exc: Exception) -> str:
    """把 DeepSeek HTTP 错误转成用户可操作的说明。"""
    if isinstance(exc, requests.HTTPError) and exc.response is not None:
        code = exc.response.status_code
        if code == 401:
            return (
                "**DeepSeek 认证失败（401 Unauthorized）**\n\n"
                "常见原因：API Key 错误、已失效、复制时多了空格/换行，或账户无可用额度。\n\n"
                "请打开 [DeepSeek 开放平台](https://platform.deepseek.com) 重新创建或复制 Key（一般以 `sk-` 开头），"
                "粘贴到侧边栏后**不要带引号**，保存后刷新页面再试。"
            )
        if code == 429:
            return "**DeepSeek 限流（429）**：请求过于频繁，请稍后再试。"
        try:
            detail = exc.response.text[:400]
        except Exception:
            detail = ""
        return f"DeepSeek 请求失败（HTTP {code}）。{detail or str(exc)}"
    return f"发生具体错误: {exc}"
...
def zhipu_ocr_image_once(*, api_key: str, image_bytes: bytes, mime: str) -> str:
    ...
    if ZhipuAI is None:
        raise RuntimeError(
            "zhipuai 无法导入，可能是依赖未安装或环境不匹配。"
            f"导入错误：{ZHIPUAI_IMPORT_ERROR}"
        )
    img_url = _encode_image_b64_url(image_bytes, mime)
...
def stream_assistant_reply() -> Optional[str]:
    ...
    with st.chat_message("assistant"):
        ...
        try:
            ...
            container.empty()
            st.error(_format_deepseek_request_error(e))
            return None
        except Exception as e:
            container.empty()
            st.error(_format_deepseek_request_error(e))
...
if __name__ == "__main__":
    main()
>>>>>>> e76c5e3b6acb5824a2a741d4cf884d8e526286c2
